...
way is to put those commands in the shell's resource file. ( For example in case of bash(1) in then I<~/.bashrc> file. )
...
Do not touch the timestamp file, I<~/.onceaday>.
...
This section describes the format of the I<onceaday.ini> file. It's name is somehow incorrect because it's format is much similar too crontab(5). With a few differences,
of course.
